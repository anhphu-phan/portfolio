...
import Modal, { ModalBody, ModalContent, ModalHeader } from "@/components/shared/Modal";
...
const imageContext = require.context(
  `@public/images/projects`, // Base directory (relative to the /public folder)
  true, // Include subdirectories
  /\.(png|jpe?g|gif)$/i, // Regular expression to match image files
);
...
export default function Project({ project }: Props) {
  ...
  const mainImage = useMemo(
    () =>
      require(`@public/images/projects/${project.imageDir}/main.png`).default as StaticImageData,
    [project.imageDir],
  );

  const imagesInProjectDir = useMemo(() => {
    return imageContext
      .keys()
      .filter((key) => key.startsWith(`./${project.imageDir}/`))
      .map((key) => imageContext(key).default as StaticImageData);
  }, [project.imageDir]);
  ...
  return (
    <>
      <div className="group relative flex w-96 flex-col gap-4 overflow-hidden rounded-xl border-2 bg-slate-100 p-8 shadow-lg">
        <Curtain>
          <Button
            className={cn("bg-transparent ring-1 ring-white hover:bg-transparent", {
              "!bg-white/20 text-blue-300 ring-blue-300": isHovered,
            })}
      ...
      <Modal show={show} onClose={handleClose} className="w-[clamp(50vw,896px,90vw)]">
        <ModalContent>
          <ModalHeader>
            <Carousel>
          ...
          <ModalBody className="space-y-6">
            ...
            <div className="flex flex-wrap gap-4">
              <h4 className="shrink-0 font-semibold">Techs used:</h4>
            ...
            <div className="space-y-2">
              <h4 className="shrink-0 font-semibold">Description:</h4>
              <ReactMarkdown
                className="prose max-w-none"
                components={{
                  a: ({ href, children }) => (
                    <Link className="text-sky-700" href={href || "#"} target="_blank">
                      {children}
                    </Link>
                  ),
                }}
